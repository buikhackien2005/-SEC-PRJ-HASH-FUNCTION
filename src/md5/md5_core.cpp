...
void MD5::transform(const uint8_t* block) {
    uint32_t M[16];
    // Đọc theo chuẩn LITTLE-ENDIAN của MD5 (Ngược hoàn toàn với SHA)
    for (int i = 0; i < 16; ++i) {
...
std::string MD5::finalize() {
    ...
    std::stringstream ss;
    // Cực kỳ lưu ý: MD5 in Hex cũng phải lật ngược byte (Little-Endian)
    for (int i = 0; i < 4; ++i) {
        ss << std::setfill('0') << std::setw(2) << std::hex << (state[i] & 0xFF)
           << std::setfill('0') << std::setw(2) << std::hex << ((state[i] >> 8) & 0xFF)
           << std::setfill('0') << std::setw(2) << std::hex << ((state[i] >> 16) & 0xFF)
           << std::setfill('0') << std::setw(2) << std::hex << ((state[i] >> 24) & 0xFF);
    }
    return ss.str();
}
